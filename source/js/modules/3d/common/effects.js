...
import {animateEasing, animateEasingWithFramerate, tick} from '../../../helpers/animation';
import getStoryShader from './shaders/story';
import {defaultHue, effectRoom} from './room-settings';
...
export const getBubbles = (canvasCenter, pixelRatio, innerHeight, innerWidth) => {
  ...
  const bubblePositionAnimationTick = (effectMaterial, index, from, to) => {
    return (progress) => {
      ...
      effectMaterial.uniforms.distortion.value.bubbles[index].position = [x, y];
    };
  };

  return {
    getUniform: (room, resolution) => {
      if (!(room.options && room.options.distort)) {
        return {};
      }

      return {distortion: {value: {bubbles, resolution}}};
    },
    animate: (effectMaterial) => {
      if (!effectMaterial.uniforms.distortion) {
        return;
      }

      effectMaterial.uniforms.options.value.distort = true;
    ...
    reset: (effectMaterial) => {
      if (!effectMaterial.uniforms.distortion) {
        effectMaterial.uniforms.options.value.distort = false;

        return;
      }

      effectMaterial.uniforms.distortion.value.bubbles.map((_, index) => {
